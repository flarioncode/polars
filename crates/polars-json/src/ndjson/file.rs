...
fn read_rows<R: BufRead>(reader: &mut R, rows: &mut [String], limit: usize) -> PolarsResult<usize> {
    ...
    for row in rows.iter_mut() {
        loop {
            ...
            let _ = reader
                .read_line(row)
                .map_err(|e| polars_err!(ComputeError: format!("{e} at line {row_number}")))?;
            if row.is_empty() {
                break;
            }
...
fn parse_value<'a>(scratch: &'a mut Vec<u8>, val: &[u8]) -> PolarsResult<BorrowedValue<'a>> {
    ...
    simd_json::to_borrowed_value(scratch).map_err(|e| polars_err!(ComputeError: format!("{e}")))
}
...
pub fn iter_unique_dtypes<R: std::io::BufRead>(
    ...
    if reader.fill_buf().map(|b| b.is_empty())? {
        // polars_err!(ComputeError: "overflow")
        polars_bail!(
            ComputeError:
